...
// we recalculate each pixel's value in an image according to a mask matrix
// (also known as kernel). This mask holds values that will adjust how much
// influence neighboring pixels (and the current pixel) have on the new pixel
// value we make a weighted average, with our specified values.

// You use the mask by putting the center of the mask matrix
// (in the upper case noted by the zero-zero index) on the pixel you want to
// calculate and sum up the pixel values multiplied with the overlapped matrix
// values.
