...
void LevelSet::initializePhi(Mat img, int iterNum, Rect boxPhi) {
    ...
    // 初始化惩罚性卷积核
    m_mK = (Mat_<float>(3, 3) << 0.5, 1, 0.5, 1, -6, 1, 0.5, 1, 0.5);

    int c = 2;
    for (int i = 0; i < m_iRow; i++) {
        for (int j = 0; j < m_iCol; j++) {
            if (i < boxPhi.y || i > boxPhi.y + boxPhi.height || j < boxPhi.x ||
                j > boxPhi.x + boxPhi.width) {
...
void LevelSet::Curvature() {
    ...
    for (int i = 0; i < m_iRow; i++) {
        ...
        for (int j = 0; j < m_iCol; j++) {
            float val =
                sqrtf(prtdx[j] * prtdx[j] + prtdy[j] * prtdy[j] + 1e-10);
            prtdx[j] = prtdx[j] / val;
            prtdy[j] = prtdy[j] / val;
        }
    }
    ...
    for (int i = 0; i < m_iterNum; i++) {
        ...
        for (int i = 0; i < m_iRow; i++) {
            ...
            for (int j = 0; j < m_iCol; j++) {
                ...
                float areaTerm =
                    fDirac * m_lambda1 * // 全局项
                    (-((fImgValue - m_FGValue) * (fImgValue - m_FGValue)) +
                     ((fImgValue - m_BKValue) * (fImgValue - m_BKValue)));

                m_mPhi.at<float>(i, j) =
                    m_mPhi.at<float>(i, j) +
                    m_timestep * (lengthTerm + penalizeTerm + areaTerm);
            }
        }

        // 显示每一次演化的结果

        // cvtColor(m_mImage, showIMG, COLOR_BGR2GRAY);
        showIMG = m_mImage.clone();
